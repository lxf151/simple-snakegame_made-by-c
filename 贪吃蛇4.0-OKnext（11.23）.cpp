...
改进:一开始静止不动,游戏不会结束 
改进：使横纵边框等比
改进:引入#define,实现一步到位创建数组,在自行修改变量时无需再更改数组 
...
#define high 25 // 对应 y坐标 0~24 	 //亲测50为极限(需要全屏) 
//--------------------------------
int len=3;//初始长度 
int x=10,y=10;//初始位置  
int add_len=2;//每吃一个fruit增加的长度
int speed=100;//调整移动速度，值越小速度越快（标准值：100） 
#define nfruit 5//fruit数量   //如果用int nfruit=5；则下面fx[nfruit]的[]中无法为 nfruit，只能是数值 
...
//int tfruit(int i,int j){//自己加的:判断多个fruit //4.0 已废用 
